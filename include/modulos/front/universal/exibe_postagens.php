<?php
function postagens($idUsuario,$tokenDia,$idTorre,$idPagina,$idBusca,$tipoBusca,$qtdSolicita,$qtdExibida,$tamanhoGrid,$ordem){
?>
        ...
        if($registro->postagem > ''){
            ...
            <div class="box slideshow col-sm-<?php echo $tamanhoGrid ?> ">
                <div class="col-sm-9 col-xs-9 text-left">
                    <p>Por:
                        <a href="/extilos/blog/<?php echo $nomeBlog?>" target="_blank"><?php echo '<i class="fa fa-user"></i>|'.$registro->arrobaUsuario ?></a></p>
                        <?php print_r($pg_conteudo); ?>
                    </div>
                    <?php if($idUsuario > 1){ ?>
                    ...
                    <div class="col-sm-12">
                        ...
                        <div class="card" id="info<?php echo $registro->postagem?>" style="display: none; height: 400px">
                            <div class="col-xs-4">
                                <p><?php echo $registro->arrobaUsuario ?></p>
                            </div>
                            <div class="col-xs-4">
                                <p><?php echo $registro->data_post ?></p>
                            </div>
                            <div class="col-xs-4">
                                <p><?php echo $registro->torreCidade ?></p>
                            </div>
                            <div class="col-xs-4">
                                <p><?php echo $registro->torreEstado ?></p>
                            </div>
                            <?php 
        ...
        <div class="col-md-12 text-center">
            <div class="pages">
                <ul class="pagination">
                    ...
                    $pag = isset($_GET['pag']) ? '&pag='.$_GET['pag'] : '';
                    $cd = isset($_GET['cd']) ? '&cd='.$_GET['cd'] : '';
                    $tp = isset($_GET['tp']) ? '&tp='.$_GET['tp'] : '';
                    $album = isset($_GET['album']) ? '&album='.$_GET['album'] : '';
                    $pgAtual = isset($_GET['pgAtual']) ? $_GET['pgAtual'] : '';
                    $pg = isset($_GET['pg']) ? $_GET['pg'] : '';

                    if(!isset($_GET['busca'])){
                    if($_GET['pg'] == 'index' or $_GET['pg'] == ''){ //USADO PARA EXIBIR BOTÃO DE VER MAIS APENAS NO INDEX
                        if($tipoBusca == 'ranking'){
                            echo '<a href="index&busca=ranking" class="btn btn-default" data-dismiss="modal">VER MAIS</a>';
                        }
                        if($tipoBusca == 'index'){
                            echo '<a href="index&busca=todos" class="btn btn-default" data-dismiss="modal">VER MAIS</a>';
                        }
                    }else{
                    $qtdPaginacao = 6;
                    ...
                    if(isset($_GET['cd']) || isset($_GET['pag'])){
                        echo '<li><a href="/extilos/'.$pg.$busca.$pag.$cd.$tp.$album.'&pgAtual=1#hot">&laquo;</a></li>';
                        for ($pgA = $paginacao[0]; $pgA < $paginacao[1]; $pgA++){
                            echo '<li><a href="/extilos/'.$pg.$busca.$pag.$cd.$tp.$album.'&pgAtual='.$pgA.'#hot">'.$pgA.'</a></li>';
                        }
                        echo '<li><a href="/extilos/'.$pg.$busca.$pag.$cd.$tp.$album.'&pgAtual='.$paginacao[2].'#hot">&raquo;</a></li>';
                    }else{
                        echo '<li><a href="/extilos/'.$_GET['pg'].'&pgAtual=1#hot">&laquo;</a></li>';
                        for ($pgA = $paginacao[0]; $pgA < $paginacao[1]; $pgA++){
                            echo '<li><a href="/extilos/'.$_GET['pg'].'&pgAtual='.$pgA.'#hot">'.$pgA.'</a></li>';
                        }
                        echo '<li><a href="/extilos/'.$_GET['pg'].'&pgAtual='.$paginacao[2].'#hot">&raquo;</a></li>';
                    }
                }
                }else{
                     $qtdPaginacao = 6;
