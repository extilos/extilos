                          ...
                          <p class="card-description">
                           ...
                           <?php while ($pagina = $paginaTorre->fetch(PDO::FETCH_ASSOC)): //prepara o conteúdo para ser listado ?>
                            ...
                            $idTorre = $pagina['idTorre'];
                                $autorizado = verifica_pg_tr($idBusca, $idTorre); //verifica se tem autorização para publicar conteúdo
                                if($autorizado['retorno'] == 'aceito'){
                                  $torre = topo_torre($pagina['idTorre']);
                                  $nomePagina = palavraCurta(20,$torre['nomeTorre']);
                                  ?>
                                  <div class="form-group">
                                    <div class="switch__container">
                                      <p><?php echo $nomePagina. $pagina['idTorre'] ?></p>
                                      <?php  if($autorizado['ativar'] > 0) { ?>
                                      <input onclick="selTorre(<?php echo $pagina['idTorre'] ?>,<?php echo $idBusca ?>)" id="<?php echo $nomePagina ?>" class="switch switch--shadow" name="check_list[]" type="checkbox" value="paginaPHP" checked>
                                      <label for="<?php echo $nomePagina ?>"></label>
                                      <?php } else {   ?>
                                      <input onclick="selTorre(<?php echo $pagina['idTorre'] ?>,<?php echo $idBusca ?>)" id="<?php echo $nomePagina ?>" class="switch switch--shadow" name="check_list[]" type="checkbox" value="paginaPHP" >
                                      <label for="<?php echo $nomePagina ?>"></label>
                                      <?php } ?>
                                    </div>
                                  </div> 
